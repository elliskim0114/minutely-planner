...
async function handleBreakdown(body: any, res: ServerResponse) {
  ...
  const system = `You are a warm planning assistant for minutely. Break down a goal into 3–6 specific focused work sessions.\n\nGoal: "${goal}"\n${totalHours ? `Total estimated hours: ${totalHours}h` : ''}\n${deadline ? `Deadline: ${deadline}` : ''}\nToday (first allowed date): ${date}\nDay hours: ${dayStart}–${dayEnd}\n\nIMPORTANT: ALL sessions must be scheduled on ${date} or later. Never schedule before ${date}.\n\nExisting blocks (avoid conflicts):\n${existingText}\n\nSpread sessions across upcoming days starting from today. Make session names specific and action-oriented. The note should be a concrete tip for that specific session.\n\nReturn a JSON object:\n{\n  "blocks": [{"name":"specific session","start":"HH:MM","end":"HH:MM","date":"YYYY-MM-DD","type":"focus","note":"brief concrete tip for this session"}],\n  "plan": "warm 2-3 sentence overview"\n}\n\nReturn ONLY the JSON object.`
  const client = getClient(apiKey)
